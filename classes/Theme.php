...
class ThemeCore extends ObjectModel
{
    ...
    public function install()
    {
        ...
        if ($xml) {
            ...
            foreach ($shops as $idShop) {
                ...
                $context->shop->id_theme = $shop->id_theme;
                $context->shop->update();
     ...
     *
     * @param string $themeDir Directory of the theme. For installed themes
     ...
     */
    public static function loadDefaultConfig($themeDir)
    {
        $themeDir = rtrim($themeDir, '/');

        $path = $themeDir.'/config.xml'; // Preferred name: all lowercase.
        if ( ! file_exists($path)) {
            // Try to find differently cased variants.
            foreach (scandir($themeDir) as $variant) {
                if (strcasecmp($variant, 'config.xml') === 0) {
                    $path = $themeDir.'/'.$variant;
                    break;
                }
            }
        }
